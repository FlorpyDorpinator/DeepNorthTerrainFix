# Thunderstore upload checklist for DeepNorthTerrainFix 1.1.3

Upload the zip `FlorpyDorp-DeepNorthTerrainFix-1.1.3.zip` at https://thunderstore.io/c/valheim/create/
...
| Version (from manifest) | 1.1.3 |
| Dependency string it will get | `FlorpyDorp-DeepNorthTerrainFix-1.1.3` |
...
2. Test once on your own server and one client: start the server, confirm the log shows
   `DeepNorthTerrainFix 1.1.3 loaded`, `Console commands registered`, and `[heal on load]` lines; type `dntf_scan`
